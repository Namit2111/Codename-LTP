    ...
    <div className="mb-8">
      ...
      <div className="space-y-4">
        {portfolioData.certifications.map((cert, index) => {
          ...
          const logoUrl = cert.company?.logo?.com?.linkedin?.common?.VectorImage?.artifacts?.[2]?.rootUrl
          ...
          return (
            <div key={index} className="bg-gray-50 p-4 rounded-lg flex items-center space-x-4 shadow-md">
              {/* Certification Logo */}
              <img
                src={logoUrl}
              ...
              <div>
                ...
                <p className="text-gray-600">{cert.company?.name || "Company Name"}</p>
                <p className="text-gray-500">
                  {cert.timePeriod?.startDate?.month}/{cert.timePeriod?.startDate?.year || "Year"}
                </p>
                <a
                  href={cert.url}
                  target="_blank"
